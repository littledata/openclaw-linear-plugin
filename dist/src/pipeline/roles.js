...
export const ROLES = {
    apex: {
        ...
        summary: "the engineering lead — you scope the work, delegate to your specialist subagents in-session, and review the result before it moves on.",
    },
    "apex-reviewer": {
        id: "apex-reviewer",
        label: "Apex Reviewer",
...
export function buildRolePrompt(role, opts) {
    ...
    if (opts.phase === "implement") {
        ...
        lines.push("You are REVIEWING, not implementing — read and analyse only, change nothing.", "The prepared ticket container is the authoritative source, including unpushed work. " +
            ...
            "Do not browse repository files through GitHub search/fetch tools.", role.verdictTag
        ...
        lines.push("Produce an implementation plan. Read the issue and the codebase, then decide", "which implementer specialists own which concerns and what each must build.", "The prepared ticket container is the authoritative codebase. Start with ONE bounded " +
