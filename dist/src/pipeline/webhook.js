    ...
    if ((payload.type === "AgentSessionEvent" && payload.action === "prompted") ||
        ...
        if (resumePending) {
            ...
            if (reply) {
                ...
                if (!decision) {
                    if (rApi)
                        await rApi.emitActivity(session.id, { type: "elicitation", body: 'Reply **resume** to continue the prior work, or **fresh** to start over.' }).catch(() => { });
                    return true;
                }
                clearResume(issue.id);
...
async function handleDispatch(api, linearApi, issue, opts) {
    ...
    if (orchestrationMode(pluginConfig) === "stateplan" && !opts?.resumeResolved && !opts?.grillDone && !getResume(issue.id)) {
        ...
        if (prior.hasPriorWork) {
            ...
            if (rsid)
                await linearApi.emitActivity(rsid, { type: "elicitation", body: ask }).catch(() => { });
            saveResume({
                issueId: issue.id,
    ...
    if ((pluginConfig?.grillMode ?? "off") === "on" && !opts?.grillDone) {
        ...
        const repoNames = Object.keys(getRepoEntries(pluginConfig));
        const step = await runGrillStep(api, { identifier, title: enrichedIssue.title ?? identifier, description: enrichedIssue.description }, repoNames, grill?.qa ?? [], resolveAgentId(api));
        if (!step.ready && step.question) {
            ...
            if (gsid)
                await linearApi.emitActivity(gsid, { type: "elicitation", body: step.question }).catch(() => { });
            saveGrill({
                issueId: issue.id,
                issueIdentifier: identifier,
                agentSessionId: gsid,
        ...
        clearGrill(issue.id);
        if (step.repos?.length)
            repoOverride = step.repos;
        if (step.guidance)
