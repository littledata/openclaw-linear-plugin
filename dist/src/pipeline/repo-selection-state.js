 ...
 */
export function savePendingRepoSelection(sel) {
    const store = read();
 ...
 */
export function getPendingRepoSelection(issueId) {
    return read()[issueId];
}
/**
 ...
 * @param issueId - the Linear issue id
 */
export function clearPendingRepoSelection(issueId) {
    const store = read();
    if (store[issueId]) {
 ...
 */
export function parseRepoSelection(reply, candidates) {
    if (/\b(all|both|everything|every\s+repo)\b/i.test(reply))
        return [...candidates];
