...
// ---------------------------------------------------------------------------
export async function checkGitHubApps(pluginConfig) {
    const checks = [];
    if (pluginConfig?.repositorySource !== "github-app") {
        checks.push(warn("Repository source is still local host mirrors", undefined, { fix: "Run: openclaw openclaw-linear github-apps migrate" }));
        return checks;
    }
    try {
        ...
        checks.push(fail("GitHub App authentication failed", err instanceof Error ? err.message : String(err), "Check both App IDs, installation IDs, PEM paths, permissions, and 0600 key modes."));
    }
    return checks;
}
// ---------------------------------------------------------------------------
