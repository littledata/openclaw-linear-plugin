 ...
 */
export function resolveRoster(pluginConfig) {
    const selected = pluginConfig?.provisionAgents;
    const ids = Array.isArray(selected) && selected.length
        ? selected.filter((x) => typeof x === "string")
        : DEFAULT_PROVISION_IDS;
    const idSet = new Set(ids.map((s) => s.toLowerCase()));
    const overrides = pluginConfig?.agentOverrides ?? {};
