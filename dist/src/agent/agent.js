...
function canonicalToolActivityName(toolName) {
...
async function runEmbedded(api, agentId, sessionId, message, timeoutMs, streaming, inactivityMs, readOnly, toolsDeny, abortKey, extraSystemPrompt) {
    ...
    let result;
    try {
        result = await embeddedAgentRuntimeScope.runInAsyncScope(() => api.runtime.agent.runEmbeddedPiAgent({
            sessionId,
            sessionTarget: {
                agentId,
                sessionId,
                    ...
                    const full = typeof data.text === "string" ? data.text
                        : typeof data.content === "string" ? data.content
                            : typeof data.message === "string" ? data.message
                                : "";
                    if (delta && !full)
                        pendingAssistantCommentary += delta;
                    else if (full.trim())
                        pendingAssistantCommentary = full;
                    return;
                }
                if (stream !== "tool")
                    return;
