...
export interface TononeSourceOptions {
  /** Git URL of the tonone repo (config `tononeRepo`). */
  repo?: string;
  /** Branch, tag, or commit SHA to check out (config `tononeRef`). */
  ref?: string;
  /** Cache directory for the checkout (config `tononeCacheDir`). */
  cacheDir?: string;
  /** Optional logger for progress. */
  logger?: { info: (m: string) => void; warn: (m: string) => void };
}

export interface TononeSource {
  /** Absolute path to the checked-out repo root. */
  dir: string;
...
export function defaultTononeCacheDir(): string {
 ...
 */
export function fetchTononeSource(options: TononeSourceOptions = {}): TononeSource {
