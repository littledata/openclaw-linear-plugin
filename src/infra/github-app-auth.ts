...
export type GitHubAppRole = "coding" | "reviewer";
...
const pendingTokens = new Map<string, Promise<CachedInstallationToken>>();

const ROLE_PERMISSIONS: Record<GitHubAppRole, Record<string, "read" | "write">> = {
  coding: {
    actions: "read",
    checks: "read",
    contents: "write",
...
export function githubAuthenticationEnvironment(token: string): Record<string, string> {
  return {
    GH_TOKEN: token,
    GIT_CONFIG_COUNT: "1",
...
export function githubAppPermissions(role: GitHubAppRole): Record<string, "read" | "write"> {
...
export function resolveGitHubAppConfig(
  ...
  const githubApps = pluginConfig?.githubApps as Record<string, unknown> | undefined;
  const raw = githubApps?.[role] as Record<string, unknown> | undefined;
  ...
  if (!appId || !installationId || !privateKeyPath) {
    throw new Error(
      `GitHub App role ${role} is not configured; expected githubApps.${role}.{appId, installationId, privateKeyPath}`,
    );
  }
...
export function invalidateGitHubAppToken(role: GitHubAppRole, repository: string): void {
  const suffix = `:${normalizeRepository(repository).toLowerCase()}`;
  for (const key of tokenCache.keys()) {
    if (key.startsWith(`${role}:`) && key.endsWith(suffix)) tokenCache.delete(key);
  }
}
...
export function parseGitHubRepositoryRemote(remote: string): string {
  ...
  if (!match) throw new Error(`Unsupported GitHub remote: ${remote}`);
...
async function createInstallationToken(
  ...
  if (!response.ok || !payload.token || !payload.expires_at) {
    throw new Error(
      `GitHub App ${role} token request failed (${response.status}): ${payload.message ?? "invalid response"}`,
    );
  }
  const expiresAtMs = Date.parse(payload.expires_at);
  if (!Number.isFinite(expiresAtMs)) throw new Error(`GitHub App ${role} token response had an invalid expiry`);
...
function readPrivateKey(path: string): string {
  ...
  if (!stat.isFile()) throw new Error(`GitHub App private key is not a regular file at ${path}`);
...
function numericIdentifier(value: unknown): number | null {
