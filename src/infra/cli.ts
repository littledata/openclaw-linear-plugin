...
import { resolveGitHubAppConfig, type GitHubAppRole } from "./github-app-auth.js";
...
async function promptGitHubAppRole(
  ...
  const installationId = await prompt(
    `  ${label} installation ID${current?.installationId ? ` [${current.installationId}]` : ""}: `,
  );
  const privateKeyPath = await prompt(
    `  ${label} PEM path on this gateway${current?.privateKeyPath ? ` [${current.privateKeyPath}]` : ""}: `,
  );
  const value = {
...
async function verifyGitHubApps(
  pluginConfig: Record<string, unknown>,
  forceRefresh = false,
): Promise<{ coding: Awaited<ReturnType<typeof listGitHubInstallationRepositories>>; reviewer: Awaited<ReturnType<typeof listGitHubInstallationRepositories>> }> {
...
export function registerCli(program: Command, api: OpenClawPluginApi): void {
  ...
  linear
    ...
    .action(async () => {
      let pluginConfig = ((api as any).pluginConfig as Record<string, unknown> | undefined) ?? {};
      ...
      const configuredApps = pluginConfig.githubApps as Record<string, Record<string, unknown>> | undefined;
  ...
  githubApps
    ...
    .action(async () => {
      ...
      try {
        ...
        const missingFromReviewer = verified.coding.filter(
          (repository) => !reviewerNames.has(repository.fullName.toLowerCase()),
        );
  ...
  agents
    ...
    .action(async () => {
      ...
      const installed = new Set(listInstalledSkills(pluginConfig.skillsInstallDir as string | undefined));
  ...
  agents
    ...
    .action(async (opts: { overwrite?: boolean }) => {
      ...
      try {
        ...
        if (result.updated.length) console.log(`  ✓ Agents updated:  ${result.updated.join(", ")}`);
        if (result.skipped.length) console.log(`  · Agents skipped:  ${result.skipped.join(", ")}`);
