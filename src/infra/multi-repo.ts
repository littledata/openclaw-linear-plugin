...
export function resolveGitHubRepository(
  ...
  if (entry?.github && /^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/.test(entry.github)) return entry.github;
  ...
  if (!/^[A-Za-z0-9_.-]+$/.test(repoName)) throw new Error(`Invalid GitHub repository name: ${repoName}`);
 ...
 */
export function detectMentionedRepos(text: string, repoNames: string[]): string[] {
  if (!text) return [];
  const found: string[] = [];
 ...
 */
export function resolveReposByNames(
  names: string[],
  pluginConfig?: Record<string, unknown>,
): RepoResolution {
  const repoMap = getRepoMap(pluginConfig);
