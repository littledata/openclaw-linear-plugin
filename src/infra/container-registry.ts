...
export interface ContainerRecord {
  ...
  /** Repo names cloned writable in the container. */
  repos: string[];
  branch: string;
  createdAtMs: number;
  /** Updated on every container tool use — the sliding-TTL anchor. */
  lastUsedMs: number;
}

type Store = Record<string, ContainerRecord>;
 ...
 */
export function setContainerRecord(rec: ContainerRecord): void {
  const store = read();
 ...
 */
export function getContainerRecord(issueIdentifier: string): ContainerRecord | undefined {
 ...
 */
export function touchContainer(issueIdentifier: string, nowMs: number = Date.now()): void {
  const store = read();
 ...
 * @param issueIdentifier - the Linear issue identifier
 */
export function removeContainerRecord(issueIdentifier: string): void {
  const store = read();
 ...
 */
export function selectIdleExpired(records: ContainerRecord[], nowMs: number, ttlMs: number): ContainerRecord[] {
