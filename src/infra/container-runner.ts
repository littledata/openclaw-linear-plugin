...
export interface ContainerStartSpec {
  ...
  reposRoot?: string;
  repositorySource: "local" | "github-app";
...
function githubEnvironmentArgs(token: string): string[] {
  return Object.entries(githubAuthenticationEnvironment(token)).flatMap(([key, value]) => ["-e", `${key}=${value}`]);
}

function isAuthenticationFailure(result: DockerResult): boolean {
  return /bad credentials|authentication failed|could not read username|http 401|401 unauthorized|token (?:has )?expired/i.test(
    `${result.stdout}\n${result.stderr}`,
  );
}

function redactToken(result: DockerResult, token: string): DockerResult {
  return {
    ...result,
...
async function runAuthenticatedDockerOperation(
  role: GitHubAppRole,
  repository: string | string[],
  pluginConfig: Record<string, unknown> | undefined,
  buildArgs: (authenticationArgs: string[]) => string[],
  timeoutMs: number,
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    if (result.status === 0 || !isAuthenticationFailure(result) || attempt === 1) return result;
    for (const scopedRepository of repositories) invalidateGitHubAppToken(role, scopedRepository);
  }
 ...
 */
export async function startOrReuseContainer(
 ...
 */
export async function ensureContainerAlive(
  identifier: string,
  repos: string[],
  branch: string,
  pluginConfig: Record<string, unknown> | undefined,
 ...
 */
export async function checkoutPullRequestInContainer(
  name: string,
  repo: string,
  ...
  return runAuthenticatedDockerOperation(
    "reviewer",
    repository,
    pluginConfig,
    (authenticationArgs) => [
 ...
 */
export async function publishPullRequestReviewInContainer(
  name: string,
  repo: string,
  pullRequestUrl: string,
  body: string,
 ...
 */
export async function execAuthenticatedInContainer(
  role: GitHubAppRole,
  name: string,
  repoNames: string[],
  command: string,
 ...
 */
export function codeSearchInContainer(
  name: string,
  repoDir: string,
  query: string,
  limit: number = 10,
  ...
  const r = dockerSync(
    ["exec", "-w", repoDir, "-e", `CCC_QUERY=${query}`, "-e", `CCC_LIMIT=${limit}`, name, "sh", "-c", CODE_SEARCH_SCRIPT],
    { timeoutMs },
  );
  return { exitCode: r.status ?? -1, stdout: r.stdout, stderr: r.stderr };
}
...
export function parseContainerGitStatus(output: string): ContainerGitStatus {
 ...
 */
export async function openPrInContainer(
  name: string,
  repoName: string,
  branch: string,
  title: string,
  ...
  const r = await runAuthenticatedDockerOperation(
    ...
    repository,
    pluginConfig,
    (authenticationArgs) => ["exec", ...authenticationArgs, ...env, name, "sh", "-c", OPEN_PR_SCRIPT],
    120_000,
  );
  ...
  if (/no commits between|nothing to compare/i.test(combined)) return null;
...
export async function execCodexInContainer(opts: ExecCodexOpts): Promise<CliResult> {
  ...
  let authenticationArgs: string[] = [];
  if (
    opts.pluginConfig?.repositorySource === "github-app" &&
    opts.githubRole &&
    ...
    try {
      installationToken = await getGitHubAppTokenForRepositories(
        opts.githubRole,
        repositories,
        opts.pluginConfig,
      );
