...
import type { ProvisionKind, ReviewStyle } from "../provisioning/agent-roster.js";

export interface WorkspacePromptOptions {
  /** Linear issue identifier (e.g. "CORE-1748"). */
  identifier: string;
  /** Repos already cloned + writable in the container: name → workdir path. */
  repos?: Array<{ name: string; workdir: string }>;
  /** The pipeline kind this agent is running as. */
  kind: ProvisionKind;
  /** Reviewer publication style (review/qa only): "comment" or "approve". */
  reviewStyle?: ReviewStyle;
  /** Reviewer leaves inline line-level comments (apex-reviewer). */
  inlineComments?: boolean;
  /** Verdict tag the orchestrator gates on (e.g. "SECURITY", "REVIEW", "QA"). */
  verdictTag?: string;
  /** Linked PR URL(s) under review (review/qa only). */
  pullRequests?: Array<{ repoName: string; url: string }>;
}
...
function reposBlock(repos: WorkspacePromptOptions["repos"]): string {
  if (!repos?.length) return "The ticket's repositories are cloned in your container workspace.";
...
function reviewPublicationBlock(opts: WorkspacePromptOptions): string {
  ...
  if (opts.reviewStyle === "approve") {
    lines.push(
      "Leave INLINE comments on the specific lines that matter, then submit an explicit verdict:",
      "1. Post line-level comments + the review in one call:",
      '   container_exec: gh api repos/<owner>/<repo>/pulls/<number>/reviews -f event=<APPROVE|REQUEST_CHANGES> \\',
      '     -f body="<summary>" -F "comments[][path]=<file>" -F "comments[][line]=<line>" -F "comments[][body]=<note>"',
      "   (repeat the comments[][...] triples per inline note; line = line number on the PR's new side).",
      "2. APPROVE when the change is correct and well-designed; REQUEST_CHANGES when it needs work.",
      "If inline line mapping fails, fall back to a review with just body + event.",
    );
  } else {
    lines.push(
      "Post your findings as a review COMMENT — do NOT approve:",
      '- Findings / non-blocking notes: container_exec: gh pr review <pr-url> --comment --body "<findings>"',
      '- A blocking problem: container_exec: gh pr review <pr-url> --request-changes --body "<why>"',
      "Never run `gh pr review --approve` — approval is not your call.",
    );
  }
...
export function buildWorkspacePrompt(opts: WorkspacePromptOptions): string {
  ...
  if (opts.kind === "plan-implement") {
    lines.push(
      "",
      "## Implementing",
      "Implement the change fully, then VERIFY by running the project's build/tests in the container. Commit in each",
      "changed repo with a structured message (summary + Changelog + Validation). Do NOT push, open a PR, or start",
      "another reviewer — the orchestrator owns push, PR creation, and review after you return control.",
    );
  } else {
    // review / qa
    lines.push(
      "",
      "## Reviewing",
      "You are REVIEWING — read and analyse only; change nothing in the code. The linked PR head is freshly checked",
      "out in your container. Prefer container_search_code to orient, then container_exec (git diff / rg / sed) to read",
      "the exact lines.",
      "",
      reviewPublicationBlock(opts),
    );
    if (opts.verdictTag) {
      lines.push(
        "",
        `End your response with EXACTLY one verdict line so the pipeline can gate:`,
        `\`${opts.verdictTag}: pass\`  or  \`${opts.verdictTag}: fail — <one-line reason>\``,
      );
    }
  }
