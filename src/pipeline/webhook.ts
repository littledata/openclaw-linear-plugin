...
export async function handleLinearWebhook(
  ...
  if (
    ...
    if (isDelegationSession) {
      ...
      const supersedesParkedInteraction = Boolean(
        getResume(issue.id) || getGrill(issue.id),
      );
  ...
  if (
    ...
    if (resumePending) {
      if (
        resumePending.agentSessionId &&
        resumePending.agentSessionId !== session.id
      ) {
        api.logger.info(
          `AgentSession prompted: ${session.id} is stale; resume gate belongs to ${resumePending.agentSessionId} — ignoring`,
        );
        return true;
      }
...
async function handleDispatch(
  ...
  if (orchestrationMode(pluginConfig) === "stateplan" && !reviewOnly) markResumeHandled(issue.id);
  ...
  if (!reviewOnly && ((pluginConfig?.grillMode as string) ?? "off") === "on" && !opts?.grillDone) {
