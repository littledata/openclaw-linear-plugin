...
import { collectReviewPullRequests, resolveReviewTargets, type ReviewTarget } from "./review-context.js";
 ...
 */
const linearSessionByIssue = new Map<string, string>();

interface PendingDelegationSessionCheck {
  token: symbol;
...
const pendingDelegationSessionChecks = new Map<string, PendingDelegationSessionCheck>();
const DEFAULT_DELEGATION_SESSION_GRACE_MS = 3_000;

function clearPendingDelegationSessionCheck(issueId: string): boolean {
...
function scheduleDelegationSessionReconciliation(
  ...
  if (pendingDelegationSessionChecks.has(issue.id)) {
    api.logger.info(
      `Issue ${issue.identifier ?? issue.id}: delegation session reconciliation already scheduled`,
    );
    return;
  }
  ...
  const timer = setTimeout(() => {
    void (async () => {
      try {
        ...
        if (nativeSession) {
          api.logger.info(
            `Issue ${issue.identifier ?? issue.id}: Linear session ${nativeSession.id} exists — fallback creation not needed`,
          );
          return;
        }
        ...
        api.logger.warn(
          `Issue ${issue.identifier ?? issue.id}: Linear did not create an AgentSession after delegation — creating fallback session`,
        );
        const result = await linearApi.createSessionOnIssue(issue.id);
        if (!result.sessionId) {
          api.logger.error(
            `Issue ${issue.identifier ?? issue.id}: fallback AgentSession creation failed: ${result.error ?? "no session ID returned"}`,
          );
          return;
        }
        ...
        api.logger.info(
          `Issue ${issue.identifier ?? issue.id}: created fallback AgentSession ${result.sessionId}; awaiting created webhook`,
        );
      } catch (err) {
        api.logger.error(
          `Issue ${issue.identifier ?? issue.id}: delegation session reconciliation failed: ${err}`,
        );
      } finally {
        const pending = pendingDelegationSessionChecks.get(issue.id);
        if (pending?.token === token) pendingDelegationSessionChecks.delete(issue.id);
      }
    })();
  }, graceMs);
  timer.unref?.();
...
export async function handleLinearWebhook(
  ...
  if (
    ...
    if (clearPendingDelegationSessionCheck(issue.id)) {
      api.logger.info(
        `AgentSession ${session.id}: native session arrived for ${issue.identifier ?? issue.id} — cancelled fallback creation`,
      );
    }
    ...
    if (specialistSession) {
      ...
      api.logger.info(
        `AgentSession ${session.id}: claimed as ${specialistSession.agentLabel} child for ${specialistSession.issueIdentifier}`,
      );
      return true;
    }
    ...
    if (isTriageSession) {
      ...
      mentionOverride = true;
      api.logger.info(
        `AgentSession ${session.id}: triage delegation for ${issue.identifier ?? issue.id} — routing to ${agentId}`,
      );
    }
    ...
    if (isDelegationSession && codingEnabled(pluginConfig) && !isTriageSession) {
      ...
      if (supersedesParkedInteraction) activeRuns.delete(issue.id);
      linearSessionByIssue.delete(issue.id);
      api.logger.info(
        `AgentSession ${session.id}: new delegation for ${issue.identifier ?? issue.id} — starting pipeline in this session`,
      );
    ...
    if (activeRuns.has(issue.id)) {
      ...
      api.logger.info(
        `AgentSession ${session.id}: dispatch active for ${issue.identifier ?? issue.id} — reusing this session, skipping conversational run`,
      );
      return true;
    }
  ...
  if (
    ...
    if (
      ...
      api.logger.info(
        `AgentSession prompted: ${session.id} is stale; grill belongs to ${grillPending.agentSessionId} — ignoring`,
      );
      return true;
    }
    ...
    if (
      activeRuns.has(issue.id) &&
      isCodexHarnessSteeringEnabled(pluginConfig as Record<string, unknown> | undefined)
    ) {
      ...
      if (userText && steeringApi && steerActiveCodexRun({
        ...
        api.logger.info(
          `AgentSession prompted: ${session.id} issue=${issue.identifier ?? issue.id} — queued on active Codex harness run`,
        );
        return true;
      }
    }
    ...
    if (tmuxSession) {
      ...
      if (
        ...
        api.logger.info(
          `AgentSession prompted: ${session.id} is stale; active work belongs to ${activeSession.agentSessionId} — ignoring`,
        );
        return true;
      }
    ...
    if (pendingRepoSel) {
      ...
      const allConfigured = Object.keys(getRepoEntries(api.pluginConfig as Record<string, unknown> | undefined));
  ...
  if (payload.type === "Issue" && payload.action === "update") {
    ...
    const routing = delegateRoutingConfig(
      pluginConfig as Record<string, unknown> | undefined,
    );
    if (routing.enabled && issue?.id && linearApi) {
      try {
        ...
        if (routed.action === "transferred") {
          api.logger.info(
            `Issue.update ${issue.identifier ?? issue.id}: ${routed.state} routes ${routed.fromOwner} → ${routed.toOwner}; delegate transferred to ${routed.delegateId}`,
          );
          return true;
        }
      } catch (err) {
        api.logger.error(
          `Issue.update ${issue.identifier ?? issue.id}: delegate routing failed: ${err}`,
        );
      }
    }
    ...
    api.logger.info(
      `Issue ${trigger} to our app user (${appUserId}) — awaiting Linear's new AgentSession.created event with fallback reconciliation`,
    );
    scheduleDelegationSessionReconciliation(
      api,
      linearApi,
      issue,
      Date.now(),
...
async function handleDispatch(
  ...
  if (reviewOnly) {
    ...
    const pullRequests = collectReviewPullRequests(
      enrichedIssue.attachments?.nodes ?? [],
      priorSessions,
      recentComments,
    );
    ...
    api.logger.info(
      `@dispatch: ${identifier} review-only state — ${reviewTargets.length} linked PR(s), repos=${repoOverride.join(",")}`,
    );
  }
  ...
  if (!reviewOnly && !opts?.contextHydrated) {
    ...
    if (prior.hasPriorWork) {
      ...
      const analysis = await synthesizePriorContext(
        api,
        {
          identifier,
      ...
      if (analysis.repos.length) priorRepoReco = analysis.repos;
      ...
      grillDone = true;
      api.logger.info(
        `@dispatch: ${identifier} hydrated context from ${prior.sessionCount} prior session(s) ` +
  ...
  if (repoOverride?.length) {
    ...
    let recoOrder: string[] = []; // LLM-ranked repos, most relevant first
    let recoReasoning = "";
    ...
    if (mustAskRepo) {
      ...
      if (selectionSessionId) {
        ...
        await linearApi.emitActivity(
          selectionSessionId,
          { type: "elicitation", body: promptIntro },
          repoSelectSignal(candidates, recoSet),
        ).catch(() => {});
      } else {
  ...
  if (!reviewOnly && ((pluginConfig?.grillMode as string) ?? "off") === "on" && !grillDone) {
    ...
    if (!step.ready && step.question) {
      ...
      if (gsid) {
        await linearApi.emitActivity(
          gsid,
          { type: "elicitation", body: step.question },
  ...
  const containersBase =
    (pluginConfig?.containersBaseDir as string) ??
  ...
  try {
    ...
    for (const target of reviewTargets) {
      ...
      if (checkout.status !== 0) {
        throw new Error(
          `could not check out ${target.url} in ${target.repoName}: ${checkout.stderr.slice(0, 300)}`,
        );
      }
  ...
  activeRuns.add(issue.id);
  let agentSessionId: string | undefined = opts?.existingSessionId ?? linearSessionByIssue.get(issue.id);
  if (!agentSessionId) {
    try {
      const sessionResult = await linearApi.createSessionOnIssue(issue.id);
  ...
  if (orchestrationMode(pluginConfig) === "stateplan") {
    ...
    try {
      ...
      const inProgress =
        teamStates.find((s: any) => s.name.toLowerCase() === "in progress") ??
        teamStates.find((s: any) => /in progress|in-progress|doing/i.test(s.name)) ??
        teamStates.find((s: any) => s.type === "started");
      if (inProgress) {
  ...
  if (!plan) {
    ...
    activeRuns.delete(issue.id);
    try { await removeActiveDispatch(identifier, statePath); } catch { /* best effort */ }
    return;
  }
  api.logger.info(`@dispatch: state-plan "${plan.stateLabel}" for ${identifier} (state="${wfState.name}")`);
  const pipelinePromise: Promise<void> = runStatePlan(hookCtx, dispatch, plan);
...
async function resumePausedDispatch(
  ...
  try {
    ...
    if (persisted.containerRepos?.length) {
      try {
        await startOrReuseContainer(
          buildContainerSpec(
            identifier,
            persisted.containerRepos,
            persisted.branch,
    ...
    const notify: NotifyFn = createNotifierFromConfig(effectivePluginConfig, api.runtime, api);
    const hookCtx: HookContext = {
      api,
      linearApi,
      notify,
