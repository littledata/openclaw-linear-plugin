...
/** How a role's work is executed. */
export type RoleBackend = "codex" | "embedded";

/** The phase-kind a role is primarily built for. */
export type RoleKind = "plan" | "implement" | "review" | "product";

export interface RoleDef {
  /** Stable id — also the agent id and Linear session-key prefix. */
  id: string;
  /** Display name shown in Linear activity. */
  label: string;
  /** OpenClaw skill name to bind (matches SKILL.md `name:` frontmatter). */
  skill: string;
  /** Default execution backend (overridable per role via config). */
  backend: RoleBackend;
  /** Deny all write-capable tools (embedded runner only). */
  readOnly: boolean;
  /** Primary phase-kind. */
  kind: RoleKind;
  /**
   * Verdict tag a reviewer emits, e.g. "SECURITY" → the last line of its output
   * must be `SECURITY: pass` or `SECURITY: fail — <reason>`. Undefined for
   * non-gating roles.
   */
  verdictTag?: string;
 ...
 */
export function resolveRole(id: string): RoleDef | undefined {
 ...
 */
export function implementerRoles(): RoleDef[] {
...
export function resolveRoleModel(
  ...
  const map = pluginConfig?.roleModels as Record<string, string> | undefined;
  const v = map?.[role.id];
...
export function resolveRoleBackend(
  ...
  const map = pluginConfig?.roleBackends as Record<string, string> | undefined;
  const v = map?.[role.id];
...
export function loadSkillGuidance(
  ...
  const dir = (pluginConfig?.skillsDir as string) || defaultSkillsDir();
  try {
...
export interface RolePromptOpts {
  /** The Linear issue identifier (e.g. "CORE-1740"). */
  identifier: string;
...
export function buildRolePrompt(role: RoleDef, opts: RolePromptOpts): string {
  ...
  if (opts.phase === "implement") {
    lines.push(
      "Implement ONLY the work assigned to you below, in the worktree provided.",
 ...
 */
export function parseReviewVerdict(output: string, tag: string): ReviewVerdict {
