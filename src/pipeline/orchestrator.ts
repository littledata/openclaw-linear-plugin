...
import { completeDispatch, updateDispatchProgress, type ActiveDispatch } from "./dispatch-state.js";
...
import { resolveTargetState, phaseAgentId, phaseKind, type StatePlan, type PlanPhase, type PhaseKind } from "./state-plan.js";
...
function codingLeadAgentId(pluginConfig?: Record<string, unknown>): string {
  return (pluginConfig?.codingLeadAgentId as string) || (pluginConfig?.implementerAgentId as string) || "apex";
}

/** Whether the Apex plan-approval gate is active (default on). */
function planApprovalEnabled(pluginConfig?: Record<string, unknown>): boolean {
  return pluginConfig?.planApprovalGate !== false;
}

/** Whether to publish the native Linear agent-session plan checklist (default on). */
function agentPlansEnabled(pluginConfig?: Record<string, unknown>): boolean {
 ...
 */
function moveTicketOnFinish(pluginConfig?: Record<string, unknown>): boolean {
 ...
 */
function unassignSelfOnFinish(pluginConfig?: Record<string, unknown>): boolean {
...
function phaseLabel(phase: PlanPhase): string {
  ...
  if (kind === "plan-implement") return "Implement";
  if (kind === "qa") return agentLabel ? `QA (${agentLabel})` : "QA";
...
interface Assignment {
  role: string;
  task: string;
  /** Ordered, concrete actions this specialist will take (the step-by-step plan). */
  steps?: string[];
}
...
interface PhaseResult {
  success: boolean;
  reason?: string;
  /** Detailed gated findings retained for the terminal session/comment handoff. */
  details?: string;
  /** Substantive review retained until the complete review bundle is published. */
  review?: ReviewRecord;
  /** Tool/sandbox/auth failure; never publish as a code verdict. */
  infrastructureFailure?: boolean;
}
    ...
    const parsed: Assignment[] = o.assignments
      ...
      .map((a: Record<string, unknown>) => ({
        ...
        steps: Array.isArray(a.steps)
          ? a.steps.map((s: unknown) => String(s).trim()).filter(Boolean)
          : undefined,
      }))
 ...
 */
function renderPlanMarkdown(assignments: Assignment[]): string {
  return assignments
    .map((a) => {
      const label = ROLES[a.role]?.label ?? a.role;
      const header = `- **${label}** — ${a.task}`;
      if (!a.steps?.length) return header;
 ...
 */
export function parsePriorSessionPlan(planText: string): Assignment[] {
  const roles = implementerRoles();
  const roleForLabel = (label: string): RoleDef | undefined =>
    roles.find((role) => role.label.toLowerCase() === label.trim().toLowerCase());
  ...
  for (const rawLine of planText.split("\n")) {
    ...
    if (current && rawLine.includes("•") && step) current.steps?.push(step);
  }
...
async function findPriorSessionAssignments(
  ctx: HookContext,
  dispatch: ActiveDispatch,
): Promise<{ assignments: Assignment[]; sessionId: string } | null> {
  if (typeof ctx.linearApi.listAgentSessions !== "function") return null;
  const sessions = await ctx.linearApi.listAgentSessions(dispatch.issueId).catch(() => []);
  for (const session of sessions) {
    if (session.id === dispatch.agentSessionId || !session.plan) continue;
    const assignments = parsePriorSessionPlan(session.plan);
    if (assignments.length) return { assignments, sessionId: session.id };
  }
  return null;
}
...
async function presentPreviousPlanForReuse(
  ctx: HookContext,
  dispatch: ActiveDispatch,
  assignments: Assignment[],
): Promise<void> {
...
async function runImplementPhase(
  ...
  const previousSessionPlan = Boolean(
    gateOn &&
  ...
  const continuationAssignment = (guidance: string): Assignment[] => [{
    role: "spine",
    task: [
  ...
  if (previousSessionPlan) {
    assignments = approval?.assignments as Assignment[];
  ...
  const planAssignments: AssignmentInput[] = assignments.map((a) => ({
    key: a.role,
  ...
  for (let attempt = 0; attempt <= limit; attempt++) {
    ...
    let beforeStatuses: Array<{ repo: string; status: ContainerGitStatus }>;
    let beforeCommits: Map<string, string>;
    try {
    ...
    if (useContainerAgent) {
      ...
      const { success, output } = await runContainerImplement(
        ctx,
        dispatch,
        issue,
        assignments,
        reworkNoteFrom(lastReviewFindings || lastReason, attempt),
      );
      ...
      for (const a of assignments) {
        ...
        const { success, output } = await runRole(
          ctx,
          dispatch,
          role,
          "implement",
    ...
    let repoStatuses: Array<{ repo: string; status: ContainerGitStatus }>;
    try {
    ...
    if (verdict.pass) {
      ...
      if (publication.success) return { success: true };
      return publication;
    }
  ...
  return shipImplementation(
    ...
    dispatch,
    issue,
    lastReason || "implementation did not pass self-review",
    lastReviewFindings,
  );
}
...
function readRepoStatuses(
  dispatch: ActiveDispatch,
): Array<{ repo: string; status: ContainerGitStatus }> {
  const repos = dispatch.containerRepos ?? [];
 ...
 */
export function hasAnyChanges(dispatch: ActiveDispatch): boolean {
  const repos = dispatch.containerRepos ?? [];
...
async function openPr(
  ...
  for (const repo of repos) {
    try {
      ...
      const prUrl = await openPrInContainer(
  ...
  if (opened.length) {
    ...
    if (!failures.length) return { success: true };
  }
  if (failures.length) {
 ...
 */
async function shipImplementation(
  ctx: HookContext,
  dispatch: ActiveDispatch,
  issue: OrchIssue,
  reason: string,
...
function reviewAuditFocus(role: RoleDef): string {
  switch (role.id) {
    case "warden":
...
async function syncReviewSandbox(ctx: HookContext, dispatch: ActiveDispatch): Promise<string | null> {
  ...
  for (const pullRequest of pullRequests) {
    const synced = await checkoutPullRequestInContainer(
      dispatch.containerName,
 ...
 */
async function publishReviewBundle(
  ctx: HookContext,
  dispatch: ActiveDispatch,
  reviews: ReviewRecord[],
  failures: Array<{ label: string; result: PhaseResult }>,
): Promise<void> {
  ...
  const agentSelfPublishes =
    isCodexHarnessSteeringEnabled(ctx.pluginConfig) && ctx.pluginConfig?.orchestratorPublishesReviews !== true;
  if (agentSelfPublishes) return;
  const pullRequests = dispatch.reviewPullRequests ?? [];
  if (!pullRequests.length || (!reviews.length && !failures.length)) return;
  if (failures.some(({ result }) => result.infrastructureFailure)) return;
  ...
  for (const pullRequest of pullRequests) {
    ...
    const published = await publishPullRequestReviewInContainer(
      dispatch.containerName,
      pullRequest.repoName,
      pullRequest.url,
      body,
  ...
  if (!publicationFailures.length) return;

  ctx.api.logger.warn(
    `[orchestrator] ${dispatch.issueIdentifier} GitHub review publication failed: ${publicationFailures.join("; ")}`,
  );
...
async function runReviewPhase(
  ...
  if (verdict.pass) return { success: true, review };
  if (!gate) {
...
// ---------------------------------------------------------------------------
// Deterministic state transition + delegate lifecycle
// ---------------------------------------------------------------------------
...
async function transitionToTarget(
  ...
  dispatch: ActiveDispatch,
  issue: OrchIssue,
  targetConfig: StatePlan["onSuccess"],
  outcome: "success" | "failure",
): Promise<TransitionResult> {
  if (!targetConfig) return { success: true };
  if (!moveTicketOnFinish(ctx.pluginConfig)) {
    ctx.api.logger.info(
      `[orchestrator] ${issue.identifier} moveTicketOnFinish=false — leaving ticket in place (${outcome} target ${JSON.stringify(targetConfig)})`,
    );
    return { success: true };
  }
  if (!issue.teamId) return { success: false, reason: "the issue has no team id for state resolution" };
  try {
...
async function releaseDelegate(
  ctx: HookContext,
  dispatch: ActiveDispatch,
  plan: StatePlan,
): Promise<void> {
  if (!unassignSelfOnFinish(ctx.pluginConfig)) {
    ctx.api.logger.info(
      `[orchestrator] ${dispatch.issueIdentifier} unassignSelfOnFinish=false — keeping delegate assigned for end-to-end continuation`,
    );
    return;
  }
  if (plan.clearDelegate === false) return;
...
async function archiveDispatch(
  ...
  if (typeof completeDispatch !== "function") return;
...
async function finishFailedPlan(
  ctx: HookContext,
  dispatch: ActiveDispatch,
  plan: StatePlan,
  issue: OrchIssue,
  ...
  const transition = await transitionToTarget(
    ctx,
    dispatch,
    issue,
    plan.onFailure ?? null,
  ...
  await endSession(
    ...
    dispatch,
    "response",
    `⛔ Blocked at ${label}: ${reason}${findings}\n\n` +
      (transition.stateName
...
export async function runStatePlan(
  ...
  ctx.api.logger.info(
    `[orchestrator] ${issue.identifier} plan=${plan.stateLabel} phases=${plan.phases.map((p) => phaseAgentId(p) ?? p.type).join(",")}`,
  );
  ...
  if (apexPhaseIndex >= 0) sessionPlan.configureApexOrchestration(apexPhaseIndex);
  // On resume, phases before the resume point already completed.
  for (let i = 0; i < startPhaseIndex; i++) sessionPlan.setPhaseStatus(i, "completed");
  ...
  try {
    const reviewFailures: Array<{ label: string; result: PhaseResult }> = [];
    ...
    for (let phaseIndex = startPhaseIndex; phaseIndex < plan.phases.length; phaseIndex++) {
      ...
      if (result.review) reviewRecords.push(result.review);
      if (!result.success) {
        ...
        if (phase.type === "review") {
          ...
          await finishFailedPlan(
            ctx,
            dispatch,
            plan,
            issue,
            reviewFailures.map((failure) => failure.label).join(" + "),
            combined,
          );
          return;
        }
    ...
    await endSession(
      ...
      dispatch,
      "response",
      `✅ Completed "${plan.stateLabel}" — all phases passed.` +
    ...
    if (!paused) disposeSessionPlan(dispatch.issueIdentifier);
  }
}
...
async function runPhase(
  ...
  if (phase.type === "review") {
    ...
    if (!role) return { success: false, reason: `unknown review agent "${agentId}"` };
    return runReviewPhase(ctx, dispatch, issue, role, phase.gate !== false, resumeGuidance, phaseKind(phase));
  }
  if (phase.type === "product") {
    const agentId = phaseAgentId(phase);
    const role = agentId ? resolveRole(agentId) : undefined;
    if (!role) return { success: false, reason: `unknown product agent "${agentId}"` };
    const focus =
      role.id === "helm"
