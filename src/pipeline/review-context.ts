...
export interface ReviewTarget extends ReviewPullRequest {
  repoName: string;
}
...
export function parseGitHubPullRequestUrl(url: string): ReviewPullRequest | null {
 ...
 */
export function collectReviewPullRequests(
  attachments: PullRequestSource[] = [],
  sessions: Array<{ pullRequests?: PullRequestSource[] }> = [],
  comments: Array<{ body?: string | null }> = [],
): ReviewPullRequest[] {
  const candidates: PullRequestSource[] = [
    ...attachments,
...
): { targets: ReviewTarget[]; unmatched: ReviewPullRequest[] } {
  const entries = getRepoEntries(pluginConfig);
  const byGithub = new Map<string, string>();
  for (const [name, entry] of Object.entries(entries)) {
    if (entry.github) byGithub.set(normalizeRepository(entry.github), name);
  }

  const targets: ReviewTarget[] = [];
  const unmatched: ReviewPullRequest[] = [];
  for (const pullRequest of pullRequests) {
    const basename = pullRequest.repository.split("/").at(-1) ?? "";
    const repoName =
      byGithub.get(normalizeRepository(pullRequest.repository)) ??
      Object.keys(entries).find((name) => name.toLowerCase() === basename.toLowerCase());
    if (repoName) targets.push({ ...pullRequest, repoName });
    else unmatched.push(pullRequest);
  }
